...
module.exports = function criarDocs(ctx) {
  const { db, saveDb, readBody, json, cuFetch, cuWrite, cuCache, pushUndo, MZ, backupAgora, usoInvalida } = ctx;
  ...
  function docCompleto(doc, sessao) {
    ...
    return {
      ...
      regras: m ? MZ.REGRAS : [], checklistMatriz: m ? MZ.CHECKLIST : [], glossario: MZ.GLOSSARIO,
  ...
  return async function rotaDocs(req, res, p, u) {
    ...
    // ---------- arquivar documentos antigos (v3.72) ----------
    // Tira do servidor os documentos de posts antigos (e os da lixeira), DEPOIS que a pessoa baixou o .zip.
    // Em 3 passos: prévia (quantos e quanto pesa), lista (o conteúdo inteiro, pro .zip ser montado no
    // navegador) e apagar (com backup antes). Documento aberto por alguém agora nunca entra.
    if (p.startsWith('/api/docs/arquivo/') && req.method === 'POST') {
      const b = await readBody(req);
      const dataDoDoc = doc => { const s = slotDe(doc); return (s && s.date) || doc.dateSnap || null; };
      const peso = doc => 450 + Buffer.byteLength(doc.html || '') + (doc.versoes || []).reduce((a, v) => a + (v.z ? v.z.length : 0) + 90, 0);
      const aberto = doc => outros(doc.id, '').length > 0;
      if (p === '/api/docs/arquivo/previa') {
        const ate = String(b.ate || '');
        if (!/^\d{4}-\d{2}-\d{2}$/.test(ate)) return json(res, 400, { erro: 'data inválida' }), true;
        const lixeira = b.lixeira !== false;
        const l = Object.values(db.docs).filter(doc => {
          if (aberto(doc)) return false;
          if (lixeira && doc.excluido) return true;
          const d = dataDoDoc(doc);
          return d ? d <= ate : String(doc.atualizadoEm || doc.criadoEm || '').slice(0, 10) <= ate;
        });
        return json(res, 200, { n: l.length, bytes: l.reduce((a, d) => a + peso(d), 0), lixeira: l.filter(d => d.excluido).length, ids: l.map(d => d.id), abertos: Object.values(db.docs).filter(aberto).length }), true;
      }
      const ids = Array.isArray(b.ids) ? b.ids.map(String).filter(id => db.docs[id]).slice(0, 5000) : [];
      if (p === '/api/docs/arquivo/lista') {
        return json(res, 200, { geradoEm: new Date().toISOString(), docs: ids.map(id => { const doc = db.docs[id]; return Object.assign({}, doc, { meta: meta(doc) }); }) }), true;
      }
      if (p === '/api/docs/arquivo/apagar') {
        if (b.confirmo !== true) return json(res, 400, { erro: 'faltou confirmar' }), true;
        if (!ids.length) return json(res, 200, { ok: true, apagados: 0, liberadoBytes: 0 }), true;
        const backup = backupAgora('antes de arquivar ' + ids.length + ' documento(s)');
        let apagados = 0, liberado = 0; const pulados = [];
        for (const id of ids) {
          const doc = db.docs[id];
          if (aberto(doc)) { pulados.push(id); continue; }
          liberado += peso(doc); apagados++;
          delete db.docs[id]; delete presenca[id];
          for (const s of db.slots) if (s.docId === id) delete s.docId;
        }
        saveDb(); if (usoInvalida) usoInvalida();
        return json(res, 200, { ok: true, apagados, liberadoBytes: liberado, pulados, backup }), true;
      }
      return json(res, 404, { erro: 'rota de arquivo desconhecida' }), true;
    }
